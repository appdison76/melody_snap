...
export default function VideoSearchScreen({ navigation, route }) {
  ...
  const handleSelectVideo = (item) => {
    ...
    try {
      const videoUrl = item.url;
      console.log('[VideoSearchScreen] Opening link-down web with from_app=1, url:', videoUrl);
      openLinkDownWithFlag(videoUrl);
    } catch (error) {
      console.error('[VideoSearchScreen] Error opening web:', error);
