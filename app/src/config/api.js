...
/** API 요청 시 서버 로그용 클라이언트 식별 헤더 */
export const getApiRequestHeaders = () => ({
  'X-Client': `MelodySnap-${Platform.OS === 'ios' ? 'iOS' : 'Android'}`,
});
...
export const openLinkDownWithFlag = (videoUrl = '') => {
  const base = LINK_DOWN_BASE_URL;
  const params = new URLSearchParams({ from_app: '1' });
  if (videoUrl && typeof videoUrl === 'string') params.set('url', videoUrl);
  const fullUrl = `${base}?${params.toString()}${LINK_DOWN_HASH}`;
  Linking.openURL(fullUrl).catch(err => console.warn('[openLinkDownWithFlag]', err));
};
